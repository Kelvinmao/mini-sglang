...
// JIT-specialized index/select kernels.
//
// Python specializes this file by element size and split count, then TVM FFI
// exports the chosen template instantiation. Each logical output row is copied
// by one or more warps instead of by scalar threads, which keeps embedding and
// projection gathers simple while still moving contiguous rows efficiently.
...
__global__ __launch_bounds__(kNumThreads, kMaxOccupancy) void //
  ...
  if (warp_id < num_warps) {
    // ``kNumSplits`` lets multiple warps cooperate on a large row. The index is
    // shared by all splits for the same logical row, while each split copies a
    // disjoint contiguous byte range from the source row.
...
__global__ __launch_bounds__(kNumThreads, kMaxOccupancy) void //
  ...
  if (warp_id < num_warps) {
    ...
    if (pos < length) {
      ...
      // Masked gathers are used for sharded embeddings: indices outside this
      // rank's vocabulary slice contribute zeros before the later all-reduce.
...
struct IndexKernel {
  static void run(const tvm::ffi::TensorView weights,
    ...
    // The device kernel sees untyped pointers because the generated
    // specialization already fixed the row byte size; dtype validation stays in
    // this host-side wrapper.
